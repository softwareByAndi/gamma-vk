...
fn context_creation_succeeds() {
    let Some(context) = skip_if_no_vulkan() else { return };
...
fn enabled_layers_accessor_works() {
    ...
    assert!(layers.is_empty() || layers.iter().all(|l| !l.is_empty()),
...
fn enabled_extensions_accessor_works() {
    ...
    #[cfg(target_os = "macos")]
    {
        ...
        println!("MoltenVK extensions status: portability_enumeration={}", 
